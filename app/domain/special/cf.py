"""
app/domain/special/cf.py
Coastal Flood scoring with flood protection subtraction.

Algorithm: identical to RF.
  1. Materialise ds[inundation_var] → tmp/CF_inundation.zarr  (breaks large dask graph)
...
def _tmp_path(stage: str) -> str:
    return f"{_tmp_base()}/CF_{stage}.zarr"
...
def score_cf(
    ...
    """
    Produce multi-scale Coastal Flood scores with flood protection subtraction.
    ...
    logger.info(f"[CF] Starting flood-protection-adjusted scoring — scales={scales}")
    ...
    if fs.exists(f"{inundation_path.removeprefix('gs://')}/.zmetadata"):
        logger.info(f"  [CF] Stage 1 skipped — reusing existing tmp: {inundation_path}")
    else:
        logger.info(f"  [CF] Stage 1: materialising {inundation_var} → {inundation_path}")
        inundation = ds[inundation_var]
        if "model" in inundation.dims:
            if inundation.sizes["model"] > 1:
                logger.info(f"  [CF] Computing ensemble mean across {inundation.sizes['model']} models")
        ...
        inundation.astype("float16").rename("inundation").to_dataset().to_zarr(inundation_path, mode="w")
        logger.info("  [CF] Stage 1 done")
    ...
    for scale in threshold_scales:
        ...
        logger.info(f"  [CF] Stage 2: scoring inundation + subtracting protection×{multiplier} (scale={scale})")
        thresholds, scores = config.get_thresholds("CF", scale)
        ...
        logger.info(f"  [CF] Stage 2 done (scale={scale})")

    if include_minmax:
        logger.info("  [CF] Stage 3: min-max normalisation + subtracting protection×20 (scale=100)")
        minmax_path = _tmp_path("minmax100")
        if fs.exists(f"{minmax_path.removeprefix('gs://')}/.zmetadata"):
            logger.info(f"  [CF] Minmax100 checkpoint exists — reusing: {minmax_path}")
        else:
        ...
        logger.info("  [CF] Stage 3 done")

    logger.info("[CF] All scales complete — concatenating results")
    return xr.concat(results, dim="scoring")


def cleanup_tmp() -> None:
    """Delete CF tmp zarrs (inundation + minmax100) after final write."""
